...
export async function GET(
  ...
  try {
    ...
    // Optional title (only used to provide a helpful fallback search URL)
    const url = new URL(req.url);
    const title = url.searchParams.get("title") || undefined;

    const result = await lookupPwcByArxiv(baseId);

    // If not found and title was provided, override searchUrl to a title search for convenience
    const payload = !result.found && title
